...
bool DataManager::loadData(const char *baseFilePath, UiManager *ui)
{
  ...
  // If both exist, we prefer encrypted, but we might want to re-encrypt plain if it's newer?
  // For now, if encrypted exists, we try to use it.

  if (SD.exists(encryptedFilePath.c_str()))
  {
    // Decrypt to temp file
    std::string tempDecryptedPath = std::string(baseFilePath) + ".tmp_dec";
    SecurityManager sec;
    if (sec.decryptFile(encryptedFilePath.c_str(), tempDecryptedPath.c_str(), ui))
    {
      dataFile = SD.open(tempDecryptedPath.c_str(), FILE_READ);
      if (dataFile)
      {
        uint16_t site = 0;
        ...
        dataFile.close();
        SD.remove(tempDecryptedPath.c_str());
        loadedData = true;
        ...
        if (ui)
        {
          ui->clear();
          ui->drawScreenBorder();
          int centerX = ui->centerXForText("Secure load OK");
          int centerY = ui->centerYForText("Secure load OK");
  ...
  if (!loadedData && SD.exists(plainFilePath.c_str()))
  {
    ...
    if (dataFile)
    {
      ...
      SecurityManager sec;
      if (sec.encryptFile(plainFilePath.c_str(), encryptedFilePath.c_str(), ui))
      {
        if (ui)
        {
          ui->print("OK");
        ...
        if (DELETE_UNENCRYPTED_FILE)
        {
          SD.remove(plainFilePath.c_str());
        }
      }
      else
      {
  ...
  // Check for .scv file (typo support or alternative extension)
  // We treat it same as .csv: load, encrypt to standard .enc, and delete source.
  std::string scvFilePath = std::string(baseFilePath);
  // Replace extension or append? baseFilePath is "passwords.csv".
  // If we want "passwords.scv", we need to replace ".csv" with ".scv" or just check "passwords.scv" if base is "passwords".
  // But base is "passwords.csv".
  // So we check if base ends in .csv and replace it.
  size_t lastDot = scvFilePath.find_last_of(".");
  if (lastDot != std::string::npos && scvFilePath.substr(lastDot) == ".csv")
  {
    scvFilePath.replace(lastDot, 4, ".scv");
  }
  else
  {
    scvFilePath += ".scv"; // Fallback
  }

  if (!loadedData && SD.exists(scvFilePath.c_str()))
  {
    dataFile = SD.open(scvFilePath.c_str(), FILE_READ);
    if (dataFile)
    {
      uint16_t site = 0;
      while (dataFile.available() && site < MaxSites)
      ...
      SecurityManager sec;
      // Encrypt to the STANDARD encrypted path (passwords.csv.enc)
      if (sec.encryptFile(scvFilePath.c_str(), encryptedFilePath.c_str(), ui))
      {
        if (ui)
        {
          ui->print("OK");
 ...
 */
bool DataManager::saveData(const char *baseFilePath, UiManager *ui)
{
  digitalWrite(TFT_CS, HIGH);
  ...
  SecurityManager sec;
  const bool success = sec.encryptFile(plainFilePathTemp.c_str(), finalEncryptedPath.c_str(), ui);
  if (success)
  {
    SD.remove(plainFilePathTemp.c_str());
  }
 ...
 * @param baseFilePath Base filename (without extension) on SD card
 */
void DataManager::deleteAllDataFiles(const char *baseFilePath)
{
  digitalWrite(TFT_CS, HIGH);

  std::string encryptedFilePath = std::string(baseFilePath) + ".enc";
  std::string plainFilePath = std::string(baseFilePath);
  ...
  if (SD.exists(plainFilePath.c_str()))
    SD.remove(plainFilePath.c_str());
  if (SD.exists(scvFilePath.c_str()))
    SD.remove(scvFilePath.c_str());
...
void DataManager::streamOut(const char *baseFilePath, std::atomic<bool> &cancelFlag, UiManager *ui)
{
  ...
  // Decrypt to temp file
  std::string tempDecryptedPath = std::string(baseFilePath) + ".tmp_out";
  SecurityManager sec;
  if (!sec.decryptFile(encryptedFilePath.c_str(), tempDecryptedPath.c_str(), ui))
  {
    if (ui)
  ...
  File dataFile = SD.open(tempDecryptedPath.c_str(), FILE_READ);
  if (dataFile)
  {
