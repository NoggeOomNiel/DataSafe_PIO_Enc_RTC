...
void DS3231_set(struct ts t)
{
    ...
    Wire1.write(DS3231_TIME_CAL_ADDR);
    for (i = 0; i <= 6; i++) {
        TimeDate[i] = dectobcd(TimeDate[i]);
        if (i == 5)
            TimeDate[5] += century;
        Wire1.write(TimeDate[i]);
    }
...
void DS3231_get(struct ts *t)
{
	...
	while(millis()-start < DS3231_TRANSACTION_TIMEOUT){
	  if (Wire1.requestFrom(DS3231_I2C_ADDR, 7) == 7) {
      	gotData = true;
      	break;
      }
      delay(2);
...
uint8_t DS3231_get_addr(const uint8_t addr)
{
	...
	uint32_t start = millis(); // start timeout
	while(millis()-start < DS3231_TRANSACTION_TIMEOUT){
	  if (Wire1.requestFrom(DS3231_I2C_ADDR, 1) == 1) {
      	gotData = true;
      	break;
      }
      delay(2);
...
float DS3231_get_treg()
{
    ...
    Wire1.beginTransmission(DS3231_I2C_ADDR);
    Wire1.write(DS3231_TEMPERATURE_ADDR);
    Wire1.endTransmission();

	uint8_t gotData = false;
	uint32_t start = millis(); // start timeout
	while(millis()-start < DS3231_TRANSACTION_TIMEOUT){
	  if (Wire1.requestFrom(DS3231_I2C_ADDR, 2) == 2) {
      	gotData = true;
      	break;
      }
      delay(2);
...
void DS3231_set_a1(const uint8_t s, const uint8_t mi, const uint8_t h, const uint8_t d, const uint8_t * flags)
{
    ...
    Wire1.beginTransmission(DS3231_I2C_ADDR);
    Wire1.write(DS3231_ALARM1_ADDR);

    for (i = 0; i <= 3; i++) {
        if (i == 3) {
            Wire1.write(dectobcd(t[3]) | (flags[3] << 7) | (flags[4] << 6));
        } else
            Wire1.write(dectobcd(t[i]) | (flags[i] << 7));
    }
...
void DS3231_get_a1(char *buf, const uint8_t len)
{
	...
	uint32_t start = millis(); // start timeout
	while(millis()-start < DS3231_TRANSACTION_TIMEOUT){
	  if (Wire1.requestFrom(DS3231_I2C_ADDR, 4) == 4) {
      	gotData = true;
      	break;
      }
      delay(2);
...
void DS3231_set_a2(const uint8_t mi, const uint8_t h, const uint8_t d, const uint8_t * flags)
{
    ...
    Wire1.beginTransmission(DS3231_I2C_ADDR);
    Wire1.write(DS3231_ALARM2_ADDR);

    for (i = 0; i <= 2; i++) {
        if (i == 2) {
            Wire1.write(dectobcd(t[2]) | (flags[2] << 7) | (flags[3] << 6));
        } else
            Wire1.write(dectobcd(t[i]) | (flags[i] << 7));
    }
...
void DS3231_get_a2(char *buf, const uint8_t len)
{
    ...
    Wire1.requestFrom(DS3231_I2C_ADDR, 4);
	uint8_t gotData = false;
	uint32_t start = millis(); // start timeout
	while(millis()-start < DS3231_TRANSACTION_TIMEOUT){
	  if (Wire1.requestFrom(DS3231_I2C_ADDR, 3) == 3) {
      	gotData = true;
      	break;
      }
      delay(2);
