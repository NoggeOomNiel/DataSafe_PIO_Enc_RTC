...
Public Class frmMain
  ...
  Private Sub SendTextToBrowserClipboardPaste(text As String)
    ...
    Dim t As New Thread(Sub()
                          Try
                            ...
                            If targetHwnd = IntPtr.Zero Then
                              BeginInvoke(Sub() Log("No browser window found to paste into."))
                              Return
                            End If
                            ...
                            Catch ex As Exception
                              BeginInvoke(Sub() Log("Clipboard set failed: " & ex.Message))
                              Return
                            End Try
                            ...
                            Catch ex As Exception
                              BeginInvoke(Sub() Log("SendKeys failed: " & ex.Message))
                            End Try
                            ...
                            BeginInvoke(Sub() Log("Pasted cell text to browser."))
                          Catch ex As Exception
                            BeginInvoke(Sub() Log("SendTextToBrowser error: " & ex.Message))
                          End Try
                        End Sub)
  ...
  ' --- New: UI Automation login-finder/filler ---

  Private Function GetTargetBrowserAutomationElement() As IUIAutomationElement
    Dim uia As New CUIAutomation8()

    Dim hwnd = ForegroundWindowIsBrowser()
    If hwnd = IntPtr.Zero Then
      hwnd = FindAnyBrowserWindowHandle()
    End If
  ...
  Delegate Sub SetValueDelegate(el As IUIAutomationElement, val As String, ByRef ok As Boolean)
  Dim setValue As SetValueDelegate

  Private Sub FillLoginOnActiveBrowser(username As String, password As String)
    If String.IsNullOrEmpty(username) AndAlso String.IsNullOrEmpty(password) Then
      Me.Invoke(Sub() Log("No username/password provided."))
      Return
    End If
    ...
    Dim t As New Thread(Async Sub()
                          Try
                            Dim uia As New CUIAutomation8()
                            Dim root = GetTargetBrowserAutomationElement()
                            If root Is Nothing Then
                              BeginInvoke(Sub() Log("No browser window available for automation."))
                              Return
                            End If

                            ' Focus browser window
                            Try
                              ForceSetForegroundWindow(root.CurrentNativeWindowHandle)
                              Thread.Sleep(150)
                            Catch
                            End Try

                            ' Find input boxes               
                            Dim editCond = uia.CreatePropertyCondition(UIA_PropertyIds.UIA_ControlTypePropertyId,
                                                       UIA_ControlTypeIds.UIA_EditControlTypeId)

                            Dim edits = root.FindAll(TreeScope.TreeScope_Descendants, editCond)

                            Dim usernameElem As IUIAutomationElement = Nothing
                            Dim passwordElem As IUIAutomationElement = Nothing

                            For i As Integer = 0 To edits.Length - 1
                              Try
                                Dim el = edits.GetElement(i)
                                Dim name = If(el.CurrentName.ToLowerInvariant(), "")
                                Dim aid = If(el.CurrentAutomationId.ToLowerInvariant(), "")
                                Dim cls = If(el.CurrentClassName.ToLowerInvariant(), "")

                                ' Username heuristics
                                If usernameElem Is Nothing AndAlso (name.Contains("user") OrElse name.Contains("email") OrElse
                                  aid.Contains("user") OrElse aid.Contains("email") OrElse
                                  cls.Contains("user") OrElse cls.Contains("email")) Then
                                  usernameElem = el
                                  Continue For
                                End If

                                ' Password heuristics
                                If passwordElem Is Nothing AndAlso
                        (name.Contains("pass") OrElse aid.Contains("pass") OrElse cls.Contains("pass")) Then
                                  passwordElem = el
                                  Continue For
                                End If
                              Catch
                            ...
                            ' Fallback ordering
                            If (usernameElem Is Nothing OrElse passwordElem Is Nothing) AndAlso edits.Length >= 1 Then
                              If edits.Length >= 2 Then
                                If usernameElem Is Nothing Then usernameElem = edits.GetElement(0)
                                If passwordElem Is Nothing Then passwordElem = edits.GetElement(1)
                              ElseIf edits.Length = 1 Then
                                If usernameElem Is Nothing Then usernameElem = edits.GetElement(0)
                              End If
                            End If

                            ' Helper to set value in an edit
                            setValue = Sub(el As IUIAutomationElement, val As String, ByRef ok As Boolean)
                                         ok = False
                                         If el Is Nothing Then Exit Sub

                                         Try
                                           Dim patternObject As Object = el.GetCurrentPattern(UIA_PatternIds.UIA_ValuePatternId)

                                           If patternObject IsNot Nothing Then
                                             Dim vp = CType(patternObject, IUIAutomationValuePattern)
                                             vp.SetValue(val)
                                             ok = True
                                           Else
                                             el.SetFocus()
                            ...
                            If usernameElem IsNot Nothing AndAlso Not String.IsNullOrEmpty(username) Then
                              setValue(usernameElem, username, userOk)
                              BeginInvoke(Sub() Log($"Username field {(If(userOk, "filled", "not filled"))}."))
                            Else
                              BeginInvoke(Sub() Log("Username field not found."))
                            End If
                            Await Task.Delay(2500)

                            ' Fill password
                            Dim passOk As Boolean = False
                            If passwordElem IsNot Nothing AndAlso Not String.IsNullOrEmpty(password) Then
                              setValue(passwordElem, password, passOk)
                              BeginInvoke(Sub() Log($"Password field {(If(passOk, "filled", "not filled"))}."))
                            Else
                              BeginInvoke(Sub() Log("Password field not found."))
                            End If
                            Await Task.Delay(1500)

                            ' Find login button
                            Dim btnCond = uia.CreatePropertyCondition(UIA_PropertyIds.UIA_ControlTypePropertyId, UIA_ControlTypeIds.UIA_ButtonControlTypeId)

                            Dim buttons = root.FindAll(TreeScope.TreeScope_Descendants, btnCond)
                            Dim loginBtn As IUIAutomationElement = Nothing

                            For i As Integer = 0 To buttons.Length - 1
                              Try
                                Dim b = buttons.GetElement(i)
                                Dim nm = If(b.CurrentName.ToLowerInvariant(), "")
                                Dim aid = If(b.CurrentAutomationId.ToLowerInvariant(), "")

                                If nm.Contains("sign in") OrElse nm.Contains("signin") OrElse
                                   nm.Contains("log in") OrElse nm.Contains("login") OrElse
                                   nm.Contains("submit") OrElse aid.Contains("login") OrElse
                                   aid.Contains("signin") Then
                                  loginBtn = b
                                  Exit For
                                End If
                              Catch
                            ...
                            ' Fallback: any element with InvokePattern that looks like login
                            If loginBtn Is Nothing Then
                              Dim invokeCond = uia.CreatePropertyCondition(UIA_PropertyIds.UIA_IsInvokePatternAvailablePropertyId, True)
                              Dim invList = root.FindAll(TreeScope.TreeScope_Descendants, invokeCond)

                              For i As Integer = 0 To invList.Length - 1
                                Try
                                  Dim el = invList.GetElement(i)
                                  Dim nm = If(el.CurrentName.ToLowerInvariant(), "")
                                  If nm.Contains("sign in") OrElse nm.Contains("log in") OrElse nm.Contains("login") Then
                                    loginBtn = el
                                    Exit For
                                  End If
                                Catch
                            ...
                            ' Click login button
                            If loginBtn IsNot Nothing Then
                              Try
                                Dim patObj As Object = loginBtn.GetCurrentPattern(UIA_PatternIds.UIA_InvokePatternId)
                                Dim invPattern = TryCast(patObj, IUIAutomationInvokePattern)

                                If invPattern IsNot Nothing Then
                                  invPattern.Invoke()
                                  'CType(patObj, IUIAutomationInvokePattern).Invoke()
                                  Me.BeginInvoke(Sub() Log("Invoked login control."))
                                Else
                                  loginBtn.SetFocus()
                                  Thread.Sleep(40)
                                  SendKeys.SendWait("{ENTER}")
                                  Me.BeginInvoke(Sub() Log("Sent ENTER to login control."))
                                End If
                              Catch ex As Exception
                                Me.BeginInvoke(Sub() Log("Login click failed: " & ex.Message))
                              End Try
                            Else
                              Me.BeginInvoke(Sub() Log("Login button not found."))
                            End If

                          Catch ex As Exception
                            Me.BeginInvoke(Sub() Log("FillLoginOnActiveBrowser error: " & ex.Message))
                          End Try
  ...
  Private Sub dgvDataSafeDataGrid_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvDataSafeDataGrid.CellMouseClick
    ...
    Try
      ...
      If col.Name = "Action1" Then
        ...
        statlabel.Text = "Attempting to fill login in active browser..."
        FillLoginOnActiveBrowser(userVal, passVal)
