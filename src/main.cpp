 ...
 *  Communicates with a host to get the current Unix time.
...
bool syncRTCWithHost()
{
    ...
    // Wait for the serial port to be ready and for the host to potentially connect
    // This loop also gives time for the user to start the vb DataSafePlus app if it wasn't already running.
...
bool askToUpdatePin()
{
    ...
    {
        // Wait for buttons to be released before starting to check
        delay(10);
    }
...
void handleSerialCommands()
{
    ...
    while (Serial.available() > 0)
    {
        ...
        if (cmd == "RESYNC") //host utillity sent a resync request
        {
            syncRTCWithHost();
            Serial.println("RTC Resynced");
        }
        else if (cmd == "REBOOT") //host utillity sent a reboot request
        {
